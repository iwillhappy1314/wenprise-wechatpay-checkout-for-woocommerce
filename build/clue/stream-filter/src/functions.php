...
namespace Wenprise\Wechatpay\Clue\StreamFilter;
 ...
 *
 * ```php
 * $filter = \Wenprise\Wechatpay\Clue\StreamFilter\append($stream, function ($chunk) {
 ...
 *
 * ```php
 * $filter = \Wenprise\Wechatpay\Clue\StreamFilter\prepend($stream, function ($chunk) {
 *     // will be called each time you read or write a $chunk to/from the stream
 *     return $chunk;
 * });
 * ```
 ...
 *
 * ```php
 * $fun = \Wenprise\Wechatpay\Clue\StreamFilter\fun('string.rot13');
 *
 ...
 *
 * ```php
 * $fun = \Wenprise\Wechatpay\Clue\StreamFilter\fun('string.strip_tags', '<a><b>');
 *
 * $ret = $fun('<b>h<br>i</b>');
 * assert('<b>hi</b>' === $ret);
 * ```
 ...
 *
 * ```php
 * $fun = \Wenprise\Wechatpay\Clue\StreamFilter\fun('zlib.deflate');
 *
 ...
 *
 * ```php
 * $filter = \Wenprise\Wechatpay\Clue\StreamFilter\append($stream, function () {
 *     // …
 * });
