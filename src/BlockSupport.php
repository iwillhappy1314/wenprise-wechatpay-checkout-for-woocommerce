...
final class BlockSupport extends AbstractPaymentMethodType {
	 ...
	 * @var string
	 */
	protected $name = WENPRISE_WECHATPAY_WOOCOMMERCE_ID;
	...
	public function is_active() {
		...
		return $payment_gateways[ WENPRISE_WECHATPAY_WOOCOMMERCE_ID ]->is_available();
	}
	...
	public function get_payment_method_script_handles() {
		$script_asset_path = WENPRISE_WECHATPAY_PATH . '/frontend/dist/blocks.asset.php';
		$script_asset      = file_exists( $script_asset_path ) ? require $script_asset_path : [ 'dependencies' => [], 'version' => WENPRISE_WECHATPAY_VERSION ];
		$script_url        = WENPRISE_WECHATPAY_URL . '/frontend/dist/blocks.js';
	...
	public function get_payment_method_data() {
		$payment_gateways_class = WC()->payment_gateways();
		$payment_gateways       = $payment_gateways_class->payment_gateways();
		$gateway                = $payment_gateways[ WENPRISE_WECHATPAY_WOOCOMMERCE_ID ];
	...
	public function failed_payment_notice( PaymentContext $context, PaymentResult &$result ) {
		if ( WENPRISE_WECHATPAY_WOOCOMMERCE_ID === $context->payment_method ) {
			add_action(
